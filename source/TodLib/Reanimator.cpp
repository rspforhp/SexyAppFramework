...
void ReanimatorEnsureDefinitionLoaded(ReanimationType theReanimType, bool theIsPreloading)
{
	...
	if (theIsPreloading)
	{
		if (gSexyAppBase->mShutdown )  // 预加载时若程序退出，则取消加载
			return;
	}
	else  // < 以下部分仅内测版执行 >
	{
			TodTraceAndLog("Failed to preload '%s'", aReanimParams->mReanimFileName);
	}  // < 以上部分仅内测版执行 >
	...
	if (aDuration > 100)  //（仅内测版）创建时间过长的报告
		TodTraceAndLog("LOADING:Long reanim '%s' %d ms", aReanimParams->mReanimFileName, aDuration);
}
